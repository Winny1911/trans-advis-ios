//
//  ArchiveProjectSubTaskCOVC.swift
//  TA
//
...
class ArchiveProjectSubTaskCOVC: BaseViewController {
    ...
    var arrofArchiveProjects: PastProjectCOProjectDetail?
    ...
    func subTaskDataFetch(){
        ...
        pastProejctsCOViewModel.getPastProjectDetailsApi(param) { model in
            self.arrofArchiveProjects = model?.data?.allProjects
//            self.subtaskTittleLbl.text = "Project Details" //"\(self.arrofArchiveProjects?.projectDetails?.title ?? "")"
            self.imgVw.sd_setImage(with: URL(string: self.arrofArchiveProjects?.projectDetails?.userData?.profilePic ?? ""), placeholderImage: UIImage(named: "Ic_profile"), completed:nil)
            self.ongoingNameLbl.text! = "\(self.arrofArchiveProjects?.projectDetails?.userData?.firstName ?? "") \(self.arrofArchiveProjects?.projectDetails?.userData?.lastName ?? "")"
            self.ongoingTittleLbl.text! = "\(self.arrofArchiveProjects?.projectDetails?.title ?? "")"
            self.ongoingDescriptionLbl.text! = "\(self.arrofArchiveProjects?.projectDetails?.projectDetailsDescription ?? "")"
//            self.ongoingAmountLbl.text = "$\(self.arrofArchiveProjects?.projectDetails?.price ?? 0)"
            
            var realAmount = "\(self.arrofArchiveProjects?.projectDetails?.price ?? 0)"
            ...
            if self.arrofArchiveProjects?.createdAt != ""{
                self.ongoingStateOnLbl.text = DateHelper.convertDateString(dateString: self.arrofArchiveProjects?.projectDetails?.createdAt ?? "", fromFormat: "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", toFormat: "dd MMM yyyy")
            }
            if self.arrofArchiveProjects?.projectDetails?.updatedAt != ""{
                self.ongoingEndOnLbl.text = DateHelper.convertDateString(dateString: self.arrofArchiveProjects?.projectDetails?.updatedAt ?? "", fromFormat: "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", toFormat: "dd MMM yyyy")
            }
            if self.arrofArchiveProjects?.projectDetails?.projectFiles?.count == 0 || self.arrofArchiveProjects?.projectDetails?.projectFiles == nil {
                self.projectFilesLabel.text = "Project Files"
            } else {
                self.projectFilesLabel.text = "Project Files (\(self.arrofArchiveProjects?.projectDetails?.projectFiles?.count ?? 0))"
            }
    ...
    @IBAction func sentButtonAction(_ sender: UIButton) {
        var id = self.arrofArchiveProjects?.projectDetails?.userData?.id ?? 0
        let user_id = String("ID_\(id ?? 0)")
        var firstName = self.arrofArchiveProjects?.projectDetails?.userData?.firstName ?? ""
        var lastName = self.arrofArchiveProjects?.projectDetails?.userData?.lastName ?? ""
        let user_name = (firstName + " " + lastName)
        let user_img = self.arrofArchiveProjects?.projectDetails?.userData?.profilePic
...
extension ArchiveProjectSubTaskCOVC: UICollectionViewDelegate, UICollectionViewDataSource{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return arrofArchiveProjects?.projectDetails?.projectFiles?.count ?? 0
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        {
            var imgStr = self.arrofArchiveProjects?.projectDetails?.projectFiles?[0].file ?? ""
            ...
            cell.projectTitleLabel.text = self.arrofArchiveProjects?.projectDetails?.projectFiles?[0].title ?? ""
            return cell
        }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
            if arrofArchiveProjects?.projectDetails?.projectFiles?.count ?? 0 > 0 {
                if self.arrofArchiveProjects?.projectDetails?.projectFiles?[0].type == "png" || self.arrofArchiveProjects?.projectDetails?.projectFiles?[0].type == "jpg" || self.arrofArchiveProjects?.projectDetails?.projectFiles?[0].type == "jpeg" {
                    if let imgStr = self.arrofArchiveProjects?.projectDetails?.projectFiles?[indexPath.row].file {
...
extension ArchiveProjectSubTaskCOVC: UITableViewDelegate,UITableViewDataSource{
...
//
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == tblVwTasks{
            ...
            cell.descriptionLbl.text = self.arrofArchiveProjects?.projectDetails?.contractorRating?.overAllFeedback
            
            var firstName = self.arrofArchiveProjects?.projectDetails?.userData?.firstName ?? ""
            var lastName = self.arrofArchiveProjects?.projectDetails?.userData?.lastName ?? ""
            cell.nameLbl.text = "\(firstName) \(lastName)"

            if let imgStr = self.arrofArchiveProjects?.projectDetails?.userData?.profilePic{

                cell.profileImages.sd_setImage(with: URL(string: imgStr), placeholderImage: UIImage(named: "doc"), completed: nil)
                }

            var rating = self.arrofArchiveProjects?.projectDetails?.contractorRating?.rating
            ...
            if self.arrofArchiveProjects?.projectDetails?.contractorRating?.ratingImages?.count != 0{
