...
class EVerificationVC: BaseViewController {
    ...
    var docURL:URL?
    var selectedType = ""
    let eVerificationViewModel: EVerificationViewModel = EVerificationViewModel()
    var projectId = 0
    var completionHandlerGoToManageBids: (() -> Void)?
    ...
    @IBAction func tapDidNextButton(_ sender: UIButton) {
        ...
        let eVerificationModel  = EVerificationModel(selectedType: selectedType)
    ...
    func setModelData(response: [String:Any]) {
        ...
        if self.inviteTask == true {
            self.submitInviteAgreement(strFile: dataDict["url"] as! String, title: randomName)
        } else{
            self.submitAgreement(strFile: dataDict["url"] as! String, title: randomName)
        }
    }
    
    func submitInviteAgreement(strFile:String, title:String) {
        if let obj = UserDefaults.standard.retrieve(object: UserProfileDataDetail.self, fromKey: TA_Storage.TA_Storage_Constants.kPersonalDetailsData) {
            ...
            eVerificationViewModel.submitAgreementApi(["agreementarr":agreementarr.toJSONString()]) { response in
                showMessage(with: "Agreement Submitted Successfully", theme: .success)
                self.navigationController?.popViewController(animated: true)
                self.completionHandlerGoToManageBids?()
                print(response!)
            }
        }
    }
...
extension EVerificationVC: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension EVerificationVC: UIDocumentPickerDelegate {
